    ...
    public class pay_line
    {
        ...
        public string Display
        {
            get
            {
                ...
                if (this.repLineNo > 0)
                {
                    string sql = "select ExportInd from pay_doc where Id='" + this.repId + "'";
        ...
        public decimal DocAmt1
        {
            get
            {
                string mastCurrency = SafeValue.SafeString(C2.Manager.ORManager.ExecuteScalar("select DocCurrency from pay_doc where Id='" + this.repId + "'"), "");
                if (mastCurrency.ToUpper() != this.currency.ToUpper())
                    return 0;
                string repTypeStr = SafeValue.SafeString(C2.Manager.ORManager.ExecuteScalar("select DocType from pay_doc where Id='" + this.repId + "'"), "");
