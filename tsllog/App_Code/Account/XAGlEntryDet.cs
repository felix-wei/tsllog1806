	...
	public class XAGlEntryDet
	{
        ...
        public string Display
        {
            get
            {
                ...
                if (this.glLineNo > 0)
                {
                    string sql = "select PostInd from XAGlEntry where SequenceId='" + glNo + "'";
                    //flag = SafeValue.SafeString(C2.Manager.ORManager.ExecuteScalar(sql), "N");
                    //if (flag == "Y")
                    //    flag = "none";

                    sql = string.Format("SELECT CloseInd FROM XXAccPeriod where Year='{0}'and  Period='{1}'", this.acYear, this.acPeriod);
                    flag = SafeValue.SafeString(C2.Manager.ORManager.ExecuteScalar(sql), "N");
                    if (flag == "Y")
                        flag = "none";
                }
