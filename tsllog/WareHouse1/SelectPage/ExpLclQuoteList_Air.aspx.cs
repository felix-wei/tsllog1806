...
public partial class PagesFreight_Account_ExpLclQuoteList_Air : System.Web.UI.Page
{
    ...
    protected void btn_Sch_Click(object sender, EventArgs e)
    {
        ...
        if (invNo.Length > 0)
        {
            string sql = string.Format("select sequenceid from AirQuote1 where  FclLclInd='LCL' and QuoteNo='{0}'", invNo);
            string sequenceId=SafeValue.SafeString(C2.Manager.ORManager.ExecuteScalar(sql),"0");
            this.dsQuotationDet.FilterExpression = "QuoteId='" + sequenceId + "'";
        }
