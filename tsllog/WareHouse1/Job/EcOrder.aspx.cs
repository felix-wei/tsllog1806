...
public partial class Modules_EcOrder : BasePage
{
    ...
    protected void submit_order(object sender, EventArgs e)
    {
        ...
        string sql = string.Format("select CustId from  [dbo].[User] where Name='{0}'", user);
        string partyId = SafeValue.SafeString(ConnectSql.ExecuteScalar(sql));
        ...
        o.PartyId = partyId;
        o.PartyName = EzshipHelper.GetPartyName(partyId);
        if(user=="MAKO"){
            o.Remark = "ONLINE ORDER";
        }
        else{
        ...
        Response.Redirect("EcOrderEdit.aspx?no=" + o.DoNo);
