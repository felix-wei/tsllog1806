...
public partial class Account_ArCashInvoiceEdit : BasePage
{
    protected void page_Init(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            Session["CashInvoiceEdit"] = null;
            if (Request.QueryString["no"] != null && Request.QueryString["no"].ToString() != "0")
            {
                ...
                Session["CashInvoiceEdit"] = "DocType='" + docType + "' and DocNo='" + Request.QueryString["no"]+"'";
            }
            else if (Request.QueryString["no"] != null && Request.QueryString["no"].ToString() == "0")
            {
                ...
                if (Request.QueryString["type"] != null)
                    docType = Request.QueryString["type"].ToString();
                this.txt_DocType.Text = docType;
                if (Session["CashInvoiceEdit"] == null)
                {
        ...
        if (Session["CashInvoiceEdit"] != null)
        {
            this.dsArInvoice.FilterExpression = Session["CashInvoiceEdit"].ToString();
    ...
    private void Save()
    {
        ...
        Session["CashInvoiceEdit"] = "SequenceId=" + inv.SequenceId;
        this.dsArInvoice.FilterExpression = Session["CashInvoiceEdit"].ToString();
        if (this.ASPxGridView1.GetRow(0) != null)
            this.ASPxGridView1.StartEdit(0);
    }
    ...
    private void UpdateMaster(int docId)
    {
        ...
        sql = string.Format("Update XAArInvoice set DocAmt='{0}',LocAmt='{1}',BalanceAmt='{2}' where SequenceId='{3}'", docAmt, locAmt, 0, docId);
        C2.Manager.ORManager.ExecuteCommand(sql);
    }
    #endregion
