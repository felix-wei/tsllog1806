...
public partial class PagesAccount_ArBatchPosting : System.Web.UI.Page
{
    ...
    protected void btn_Post_click(object sender, EventArgs e)
    {
        ...
        if (where.Length > 0)
        {
            ...
            bool arCashInvoice = this.cb_ArCashInvoice.Checked;
            if (arCashInvoice)
            {
                PostArCashInvoiceList(where);
            }
            ...
            bool apCn = this.cb_ArPc.Checked;//ar refund
            if (apCn)
            {
                PostArPcList(where);
            }
            Response.Write("Post complete");
    ...
    {
        string sql = "Select SequenceId,DocNo,DocType,ExportInd from XAArInvoice where Isnull(ExportInd,'N')='N' and Isnull(CancelInd,'N')='N' and  (DocType='IV' or DocType='DN') and " + where;
