<%@ Page Language="C#" AutoEventWireup="true" EnableViewState="false" CodeFile="EcOrderStaff.aspx.cs" Inherits="Modules_EcOrderStaff" %>
...
//from ref_Product p 
//			");
            string sql = string.Format(@"select DISTINCT p.Att4 as Att1, p.Att5 as Att2, p.Att6 as Att3, p.Att7 as Att4, p.Att8 as Att5, p.Att9 as Att6, p.Description,p.Code as ProductCode,ISNULL(tab_Incoming.Qty,0) as IncomingQty,ISNULL(d2_In.Qty,0) as InQty,
  p.QtyPackingWhole as QtyPackWhole, ISNULL(d2_Out.qty,0) as OutQty,'' as LotNo,0 as Qty1,
ISNULL((select sum(det.Qty1) from Wh_TransDet det left join Wh_Trans mast on det.DoNo=mast.DoNo and mast.DoStatus='Confirmed' where det.ProductCode=d1.ProductCode and mast.DoType='SO' group by ProductCode),0) as WipQty, 
ISNULL((select sum(det.Qty1) from Wh_TransDet det left join Wh_Trans mast on det.DoNo=mast.DoNo and mast.DoStatus='Confirmed' where det.ProductCode=d1.ProductCode  and mast.DoType='PO' group by ProductCode),0) as WipQtyIn, 
ISNULL((select sum(wd.Qty1) from Wh_TransDet wd, Wh_Trans wt where wt.DoType='PO' and wt.DoStatus='Draft' and wt.DoNo=wd.DoNo and wd.ProductCode=d1.ProductCode),0) as PoQty,
ISNULL((select sum(wd.Qty1) from Wh_TransDet wd, Wh_Trans wt where wt.DoType='SO' and wt.DoStatus='Draft' and wt.DoNo=wd.DoNo and wd.ProductCode=d1.ProductCode),0) as SoQty,
isnull((select top(1) price from Wh_TransDet det inner join Wh_Trans mast on det.DoNo=mast.DoNo and det.DoType=mast.DoType where mast.DoType='SO' and det.ProductCode=d1.ProductCode order by mast.DoDate desc),0) as LastSell,
isnull((select top(1) price from Wh_TransDet det inner join Wh_Trans mast on det.DoNo=mast.DoNo and det.DoType=mast.DoType where mast.DoType='PO'and det.ProductCode=d1.ProductCode order by mast.DoDate Desc),0) as LastBuy 
from ref_product p  
left join Wh_DoDet as d1  on d1.ProductCode= p.Code
left join Wh_DO d3 on d1.DoNo=d3.DoNo
left join (select Product,sum(qty1) qty from Wh_DoDet2 det inner join wh_do mast on mast.DoNo=det.DoNo and mast.StatusCode='CLS'  where det.DoType='IN' and len(det.DoNo)>0  group by Product) as d2_In on d2_In.Product=d1.ProductCode
left join (select Product,sum(qty1) qty from Wh_DoDet2 det inner join Wh_DO mast on det.DoNo=mast.DoNo and (mast.StatusCode='CLS' or mast.StatusCode='RETURN') where det.DoType='Out' and len(det.DoNo)>0 group by Product) as d2_Out on d2_Out.Product=d2_In.Product
left join (select ProductCode,sum(Qty5) as Qty from Wh_DoDet where DoType='IN' group by ProductCode) as tab_Incoming on tab_Incoming.ProductCode=p.Code 
 order by p.Code asc
			");
  ...
  p.QtyPackingWhole as QtyPackWhole, ISNULL(d2_Out.qty,0) as OutQty, '' as LotNo,0 as Qty1,tab_Po.PoQty,tab_So.SoQty,tab_block.WipQty,tab_WipQtyIn.WipQtyIn
from Wh_DoDet as d1 inner join Wh_DO d3 on d1.DoNo=d3.DoNo left join ref_product p on p.Code=d1.ProductCode
...
where d1.DoType='IN' and (PoQty>0 or SoQty>0 or tab_Incoming.Qty>0 or WipQty>0) order by p.Code asc
...
<%--                    <input type="hidden" name="line<%= i+1 %>p" value='<%= dr_sum["ProductCode"] %>' />
            ...
            <% } %>
                    ...
                    <table>
                        <tr>
                            <td>
                                <dxe:ASPxButton ID="btn_search" Width="150" runat="server" Text="Create Sales" onclick="submit_order" />
                            </td>
                              <td>
                                <dxe:ASPxButton ID="btn_purchase" Width="150" runat="server" Text="Create Purchase"  OnClick="btn_purchase_Click">
                                    </dxe:ASPxButton>
                            </td>
                            <td>
                        ...
                        window.location='EcOrderStaff.aspx';
                             ...
                             <td>
                                 <dxe:ASPxButton ID="ASPxButton3" Width="150" runat="server" Text="Search Sales" AutoPostBack="false"
                            ...
                            <ClientSideEvents Click="function(s,e) {
                             ...
                             <td>
                                 <dxe:ASPxButton ID="ASPxButton1" Width="150" runat="server" Text="Search Purchase" AutoPostBack="false"
                            UseSubmitBehavior="false">
                            <ClientSideEvents Click="function(s,e) {
        ...
        <table border="2" style="border-collapse:collapse" width="900">
            ...
            <tr>
                ...
                <th colspan="2" class="price">Costing Price</th>
            </tr>
            <tr>
                <th>#</th>
                <th>CODE</th>
                ...
                <th class="price">LAST SELL</th>
                <th class="price">LAST BUY</th>
            </tr>
            <%
            ...
            <tr >
                ...
                <td class="qty"><input type="text" name="line<%= i+1 %>r" width="6" />
                </td>
                <td><%= dr["LastSell"] %></td>
                <td><%= dr["LastBuy"] %></td>
            </tr>
