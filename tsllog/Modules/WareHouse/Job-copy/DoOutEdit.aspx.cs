...
public partial class Pages_DoOutEdit : System.Web.UI.Page
{
    ...
    protected string Save()
    {
        try
        {
            ...
            ASPxButtonEdit txt_TransportName = grid_Issue.FindEditFormTemplateControl("txt_TransportName") as ASPxButtonEdit;
            whDo.TransportName= txt_TransportName.Text;
            ASPxSpinEdit spin_TransportFee = grid_Issue.FindEditFormTemplateControl("spin_TransportFee") as ASPxSpinEdit;
            whDo.TransportFee = SafeValue.SafeDecimal(spin_TransportFee.Text);
...
where DoNo='{0}'", refN.Text);
            ...
            else
            {
                ...
                else
                {
                    ...
                    //    return;
                    //}
                    //sql = string.Format(@"select sum(CostLocAmt) from Wh_Costing where RefNo='{0}' and JobType='OUT'",refN.Text);
...
from Wh_DoDet2 where Id='{0}'", id, EzshipHelper.GetUserName());
        ...
        else if (s == "Costing")
        {
            ...
            String[] chgCode = { "BGFEE", "HANDLE", "PERMIT", "STUFF20", "STUFF40", "ASN", "UNIQUE", "PALLET", "TPT", "FORKLIFT", "STORE", "CUSTOMS", "MISC", "ADHOC" };
...
and wh_dodet2.doNo='{0}'", orderNo);
    ...
    protected void grid_SKULine_RowDeleting(object sender, DevExpress.Web.Data.ASPxDataDeletingEventArgs e)
    {
        ...
        if (IsShipped(refN.Text, product))
        {
            throw new Exception("Had Picked,Can not delete !!!");
            return;
        }
        else
        {
