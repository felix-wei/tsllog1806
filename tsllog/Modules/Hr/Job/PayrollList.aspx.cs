...
public partial class Modules_Hr_Job_PayrollList : System.Web.UI.Page
{
    ...
    {
        string person = SafeValue.SafeString(txtSchName.Value, "");
        string sql_where = "";
        string dateFrom = "";
        string dateTo = "";
        string sql = string.Format(@"select r.*, (isnull(total1,0)-isnull(total2,0)-isnull(cpf2amt,0) ) as NetPay from Hr_Payroll r inner join Hr_Person p on r.Person=p.Id ");
        if (person.Length > 0)
        {
            sql_where = String.Format("r.Person ='{0}'", person);
        }
        ...
        if(1==1)
        {
            if (txt_from.Value != null && txt_end.Value != null)
            {
                ...
                sql_where = GetWhere(sql_where, string.Format("r.ToDate >= '{0}' and r.FromDate <= '{1}'", dateFrom, dateTo));
            }
            if (SafeValue.SafeString(cmb_Department.Value).Length > 0)
            {
                sql_where = GetWhere(sql_where, string.Format("p.Department='{0}'", SafeValue.SafeString(cmb_Department.Value)));
            }
        }
        if (sql_where.Length > 0)
        {
            sql = sql + " where " + sql_where + "  order by p.Name ";
        }
...
decimal rate1 = 0;
    ...
    private decimal Get_Incentive(string name)
    {
        ...
        string dateTo = txt_end.Date.ToString("yyyy-MM-dd");
