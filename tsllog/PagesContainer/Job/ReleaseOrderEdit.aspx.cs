...
public partial class PagesContainer_Job_ReleaseOrderEdit : System.Web.UI.Page
{
    protected void Page_Init(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["no"] != null && Request.QueryString["no"].ToString() != "0")
            {
                ...
                //this.txt_TptNo.Text = Request.QueryString["no"].ToString();
    ...
    protected void grid_Transport_InitNewRow(object sender, DevExpress.Web.Data.ASPxDataInitNewRowEventArgs e)
    {
        ...
        e.NewValues["ShipPol"] = System.Configuration.ConfigurationManager.AppSettings["LocalPort"];
    }
    protected void SaveJob()
    {
        try
        {
            ...
            if (Id == 0)
            {
                isNew = true;
                contA = new ContAsset();
                contA.DocNo = C2Setup.GetNextNo("ReleaseOrder");
                contA.DocType = "RO";
            }
            ...
            ASPxComboBox releaseType = g.FindEditFormTemplateControl("cmb_ReleaseType") as ASPxComboBox;
            contA.ReleaseType = releaseType.Text;
            ...
            ASPxTextBox haulierName = g.FindEditFormTemplateControl("txt_HaulierName") as ASPxTextBox;
            contA.HaulierRef = haulierName.Text;
            ...
            if (isNew)
            {
                ...
                C2Setup.SetNextNo("ReleaseOrder", contA.DocNo);
    ...
    private string close_Asset()
    {
        ASPxTextBox masterId = this.grid_Transport.FindEditFormTemplateControl("txt_Id") as ASPxTextBox;
        string sql = "update Cont_Asset set StatusCode=case when statusCode='CLS' then 'USE' else 'CLS' end where Id=" + masterId.Text;
        int res = Manager.ORManager.ExecuteCommand(sql);
        if (res > 0)
        {
            return "Success";
        }
        return "Can not close, Check again ";

    }
    private string void_Asset()
    {
        ASPxTextBox masterId = this.grid_Transport.FindEditFormTemplateControl("txt_Id") as ASPxTextBox;
        string sql = "update Cont_Asset set StatusCode=case when statusCode='CNL' then 'USE' else 'CNL' end where Id=" + masterId.Text;
        int res = Manager.ORManager.ExecuteCommand(sql);
        if (res > 0)
        {
            return "Success";
        }
        return "Can not void, Check again ";
    }
