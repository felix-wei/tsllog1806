...
public partial class SelectPage_ApList : System.Web.UI.Page
{
    ...
    protected void btn_Sch_Click(object sender, EventArgs e)
    {
        ...
        string supplerBillNo = this.txt_SupplierBillNo.Text.Trim();
        string sql = string.Format("select sequenceid from XaApPayable where DocNo='{0}'", invNo);
        if (invNo.Length > 0)
        {
            sql = string.Format("select sequenceid from XaApPayable where DocNo='{0}'", invNo);
        }
        else if (supplerBillNo.Length > 0)
        {
            sql = string.Format("select sequenceid from XaApPayable where SupplierBillNo='{0}'", invNo);
        }
        if (sql.Length > 0)
        {
            string sequenceId = SafeValue.SafeString(C2.Manager.ORManager.ExecuteScalar(sql), "0");
            this.dsApDet.FilterExpression = "DocId='" + sequenceId + "'";
        }
    }
    protected void grid_CustomDataCallback(object sender, DevExpress.Web.ASPxGridView.ASPxGridViewCustomDataCallbackEventArgs e)
    {
        if (Request.QueryString["id"] != null )
        {
            ...
            int index = SafeValue.SafeInt(C2.Manager.ORManager.ExecuteScalar(string.Format("Select max(QuoteLineNo) FROM SeaApQuoteDet1 where QuoteId='{0}'", invId)), 1);
            for (int i = 0; i < list.Count; i++)
            {
                try
                {
                    ...
                    string sql = string.Format(@"SELECT ChgCode,ChgDes1, Currency,ExRate, Price, Unit, GstType, Gst FROM XaApPayableDet where SequenceId='{0}' ", sequenceId);
                    ...
                    if (tab.Rows.Count == 1)
                    {
                        ...
                        SeaApQuoteDet1 det = new SeaApQuoteDet1();
                        det.Amt =0;
                        det.ChgCode=chgCode;
                        det.ChgDes = chgDes;
