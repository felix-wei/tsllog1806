...
public partial class PagesFreight_SelectPage_StdRateFcl_multi : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                this.dsSeaApQuoteDet.FilterExpression = " FclLclInd='Fcl' and  QuoteId='-1'";
            }
            else
                this.dsSeaApQuoteDet.FilterExpression = "1=0";
        }
        OnLoad();
    }
        ...
        if (Request.QueryString["no"] != null)
        {
            ...
            try
            {
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    string sql = string.Format("SELECT ChgCode,ChgDes, Currency, ExRate,Qty,Price, Unit, MinAmt, Rmk,GstType,Gst,Amt FROM SeaApQuoteDet1 where SequenceId='{0}'", docId);
                    ...
                    if (tab.Rows.Count == 1)
                    {
                        ...
                        C2.SeaApQuoteDet1 repDet = new SeaApQuoteDet1();
                        ...
                        repDet.Gst = gst;
                        repDet.Amt = amt;
                        string sql_detCnt = "select max(QuoteLineNo) from SeaApQuoteDet1 where QuoteId='" + repId + "'";
