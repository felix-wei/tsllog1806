...
public partial class PagesFreight_Account_ExpFclApQuoteList_Sea : System.Web.UI.Page
{
    ...
    protected void btn_Sch_Click(object sender, EventArgs e)
    {
        ...
        if (invNo.Length > 0)
        {
            string sql = string.Format("select sequenceid from SeaApQuote1 where  FclLclInd='FCL' and QuoteNo='{0}'", invNo);
            string sequenceId=SafeValue.SafeString(C2.Manager.ORManager.ExecuteScalar(sql),"0");
            this.dsApQuotationDet.FilterExpression = "QuoteId='" + sequenceId + "'";
        }
            ...
            for (int i = 0; i < list.Count; i++)
            {
                try
                {
                    ...
                    string sql = string.Format(@"SELECT ChgCode, Currency,ExRate, Price, Unit, MinAmt, Rmk, Qty, Amt, GstType, Gst, GroupTitle, 
                      ChgDes FROM SeaApQuoteDet1 where SequenceId='{0}' order by QuoteLineNo", sequenceId);
