...
public partial class Page_Upload : System.Web.UI.Page
{
    ...
    public void AddFile(string type, string code, string _name, string desc,string newName)
    {
        C2.TptAttachment photo = new C2.TptAttachment();
        photo.JobType = Request.QueryString["Type"].ToString();
        photo.FileName = _name;
        photo.RefNo = code;
        //photo.conterno = cmb_ContNo.Text;
        photo.JobNo = cmb_DoNo.Text;
        photo.FileNote = desc;
        photo.FileType = type;
        photo.FilePath = GetFilePath(this.txt_Sn.Text, this.cmb_ContNo.Text, this.cmb_DoNo.Text) + newName;
