...
public partial class Pages_ImportList : System.Web.UI.Page
{
    ...
    protected void btn_search_Click(object sender, EventArgs e)
    {
        ...
        else if (this.txt_AgtId.Text.Length > 0)
        {
            ...
            if (dateFrom.Length > 0 && dateTo.Length > 0)
                where += string.Format(" and Eta >= '{0}' and Eta < '{1}'", dateFrom, dateTo);
            string sql = string.Format("SELECT job.JobNo FROM SeaImport AS job INNER JOIN SeaImportRef AS ref ON job.RefNo = ref.RefNo WHERE {0}",where);
        ...
        {
            where += string.Format(" Eta >= '{0}' and Eta < '{1}'", dateFrom, dateTo);
            string sql = string.Format("SELECT job.JobNo FROM SeaImport AS job INNER JOIN SeaImportRef AS ref ON job.RefNo = ref.RefNo WHERE {0}", where);
            DataTable tab = Manager.ORManager.GetDataSet(sql).Tables[0];
            string where1 = "( 1=0";
    ...
    {
        string where = "(TsInd = 'Y') AND (ISNULL(TsBkgId, '') = '')";
        ...
        if (txt_RefNo.Text.Trim() != "")
            where += " and RefNo='" + txt_RefNo.Text.Trim() + "'";
        else if (this.txt_HouseNo.Text.Length > 0)
        {
            where += " and JobNo='" + this.txt_HouseNo.Text.Trim() + "'";
        }
        else if (this.txt_HblN.Text.Length > 0)
        {
            where += " and HblNo='" + this.txt_HblN.Text.Trim() + "'";
        }
        else if (this.txt_AgtId.Text.Length > 0)
        {
            where += " and CustomerId='" + this.txt_AgtId.Text.Trim() + "'";
