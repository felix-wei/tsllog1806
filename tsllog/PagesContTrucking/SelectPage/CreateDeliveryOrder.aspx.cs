...
public partial class PagesContTrucking_SelectPage_CreateDeliveryOrder : System.Web.UI.Page
{
...
left join (select  sum(case when CargoType='IN' then QtyOrig else -QtyOrig end) as BalQty,ClientId,SkuCode,BookingNo,Location from job_house where CargoStatus='C' group by ClientId,SkuCode,BookingNo,Location) as tab_bal on tab_bal.ClientId=mast.ClientId and tab_bal.SkuCode=mast.SkuCode and tab_bal.BookingNo=mast.BookingNo and tab_bal.Location=mast.Location
where mast.ClientId='{0}' and mast.SkuCode='{1}' and mast.BookingNo='{2}' and mast.JobNo='{3}' and mast.Location='{4}'", client, code, bookingNo, SafeValue.SafeString(refNo), loc);
    ...
    {
        string sql = string.Format(@"select tab_bal.BalQty  from job_house mast
left join (select  sum(case when CargoType='IN' then PackQty else -PackQty end) as BalQty,ClientId,SkuCode,BookingNo,Location from job_house where CargoStatus='C' group by ClientId,SkuCode,BookingNo,Location) as tab_bal on tab_bal.ClientId=mast.ClientId and tab_bal.SkuCode=mast.SkuCode and tab_bal.BookingNo=mast.BookingNo and tab_bal.Location=mast.Location
where mast.ClientId='{0}' and mast.SkuCode='{1}' and mast.BookingNo='{2}' and mast.JobNo='{3}' and mast.Location='{4}'", client, code, bookingNo, refNo, loc);
        return SafeValue.SafeDecimal(ConnectSql.ExecuteScalar(sql));
    }
    public decimal BalanceWeight(object client, object code, object bookingNo, object refNo, object loc)
    {
        string sql = string.Format(@"select tab_bal.BalQty  from job_house mast
left join (select  sum(case when CargoType='IN' then WeightOrig else -WeightOrig end) as BalQty,ClientId,SkuCode,BookingNo,Location from job_house where CargoStatus='C' group by ClientId,SkuCode,BookingNo,Location) as tab_bal on tab_bal.ClientId=mast.ClientId and tab_bal.SkuCode=mast.SkuCode and tab_bal.BookingNo=mast.BookingNo and tab_bal.Location=mast.Location
where mast.ClientId='{0}' and mast.SkuCode='{1}' and mast.BookingNo='{2}' and mast.JobNo='{3}' and mast.Location='{4}'", client, code, bookingNo, refNo, loc);
        return SafeValue.SafeDecimal(ConnectSql.ExecuteScalar(sql));
    }
    ...
    {
        string sql = string.Format(@"select tab_bal.BalQty  from job_house mast
left join (select  sum(case when CargoType='IN' then VolumeOrig else -VolumeOrig end) as BalQty,ClientId,SkuCode,BookingNo,Location from job_house where CargoStatus='C' group by ClientId,SkuCode,BookingNo,Location) as tab_bal on tab_bal.ClientId=mast.ClientId and tab_bal.SkuCode=mast.SkuCode and tab_bal.BookingNo=mast.BookingNo and tab_bal.Location=mast.Location
where mast.ClientId='{0}' and mast.SkuCode='{1}' and mast.BookingNo='{2}' and mast.JobNo='{3}' and mast.Location='{4}'", client, code, bookingNo, refNo, loc);
        return SafeValue.SafeDecimal(ConnectSql.ExecuteScalar(sql));
    }
