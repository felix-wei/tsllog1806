...
<%
    ...
    string sql_sum  =  @"select Category, sum(LocAmt1) as LocAmt1, Sum(GstAmt) as GstAmt, Sum(Gst2Amt) as Gst2Amt, Sum(LocAmt2) as LocAmt2 from (
            ...
            + " union all " + 
            "select '2.Expense' as Category,0 as LocAmt1, 0 as LocAmt,  0 as LocAmt2, 0 as GstAmt, -1*sum(charge1+charge2+charge3+charge4+charge5+charge6+charge7+charge8+charge9+charge10) as Gst2Amt from ctm_jobdet2 l where l.jobno='" + job_no + "' " 
            + " union all " + 
            "select '2.Expense' as Category, 0 as LocAmt1,0 as LocAmt, -1*sum(incentive1+incentive2+incentive3+incentive4) as LocAmt2, 0 as GstAmt, 0 as Gst2Amt from ctm_jobdet2 l where l.jobno='" + job_no + "' ) as Tmp group by Category  " 
		;
...
<%
	...
	incentive1+incentive2+incentive3+incentive4 as driver,
	charge1,charge2,charge3,charge4,charge5,charge6,charge7,charge8,charge9,charge10,
	charge1+charge2+charge3+charge4+charge5+charge6+charge7+charge8+charge9+charge10 as claim
	from ctm_jobdet2 where jobno='" + job_no + "' order by containerno, id");
