...
<script runat="server">
    ...
    void Page_Init(object o, EventArgs e)
    {
      ...
      master.report_name = "Export Shipment Performance CBM Analysis";
      master.drilldown_list = new string[] { "AGENT","CUST","JOBTYPE", "ETA", "PORT", "STAFF", "Wt/M3" };
    ...
    master.query_sql = @"SELECT    m.CreateBy AS STAFF, m.Pod AS PORT, p.Code AS AGENT, m.jobtype AS JOBTYPE, CONVERT(nvarchar(4), 
                      ...
                      c.Volume AS VOLUME, CONVERT(varchar(20), c.Weight)+'/'+CONVERT(varchar(20),c.Volume) AS [Wt/M3]
FROM         SeaExport AS c INNER JOIN
...
WHERE  m.statuscode<>'CNL' and  (CONVERT(DATETIME, m.Eta, 111) BETWEEN '{0}' AND '{1}')";
    master.filename_template = "Export_CBM_{0:yyyy}_{1:MM}_{2:dd}.xls";
