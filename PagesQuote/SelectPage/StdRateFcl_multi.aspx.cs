...
public partial class PagesFreight_SelectPage_StdRateFcl_multi : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["no"] != null)
            {
                ...
                string filter=" FclLclInd='Fcl' and  QuoteId='-1' and ImpExpInd='"+impExpInd+"'";
                if (Request.QueryString["pol"] != null)
                {
                    string pol = Request.QueryString["pol"].ToString();
                    filter = filter + " and (isnull(ltrim(Pol),'')=''" + " or Pol='" + pol + "')";
                }
                if (Request.QueryString["pod"] != null)
                {
                    string pod = Request.QueryString["pod"].ToString();
                    filter = filter + " and (isnull(ltrim(Pod),'')=''" + " or Pod='" + pod + "')";
                }
                if (Request.QueryString["partyTo"] != null)
                {
                    string partyTo = Request.QueryString["partyTo"].ToString();
                    filter = filter + " and (isnull(ltrim(PartyTo),'')=''" + " or PartyTo='" + partyTo + "')";
                }
                this.dsSeaQuoteDet.FilterExpression = filter;
            }
            else
                this.dsSeaQuoteDet.FilterExpression = "1=0";
        }
        OnLoad();
    }
