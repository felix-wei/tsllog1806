...
public partial class PagesFreight_ExpFclApQuoteEdit : BasePage
{
    protected void page_Init(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            Session["SeaApQuoteEditWhere"] = null;
            if (Request.QueryString["no"] != null && Request.QueryString["no"].ToString() != "0")
            {
                this.txtSchNo.Text = Request.QueryString["no"].ToString();
                Session["SeaApQuoteEditWhere"] = "QuoteNo='" + SafeValue.SafeString(Request.QueryString["no"], "")+"'";

                if (role == "SALESSTAFF")
                {
                    DataTable tab = C2.Manager.ORManager.GetDataSet(@"SELECT q.QuoteNo FROM SeaApQuote1 AS q INNER JOIN XXParty AS c ON q.PartyTo = c.PartyId
WHERE     (c.SalesmanId = '" + userName + "') and QuoteNo='" + SafeValue.SafeString(Request.QueryString["no"], "") + "'").Tables[0];
    ...
    protected void ASPxGridView1_Init(object sender, EventArgs e)
    {
        ...
        if (grd != null)
            grd.ForceDataRowType(typeof(C2.SeaApQuote1));
    }
    protected void ASPxGridView1_InitNewRow(object sender, DevExpress.Web.Data.ASPxDataInitNewRowEventArgs e)
    {
        e.NewValues["Title"] = "FCL Sea ApQuotation";
    ...
    protected void grid_InvDet_Init(object sender, EventArgs e)
    {
        ASPxGridView grd = sender as ASPxGridView;
        if (grd != null)
            grd.ForceDataRowType(typeof(C2.SeaApQuoteDet1));
    }
