...
public partial class Pages_ExportRefList : System.Web.UI.Page
{
    ...
    protected void btn_search_Click(object sender, EventArgs e)
    {
        ...
        string sql = @"select job.SequenceId,ref.RefNo,ref.OblNo as OBL,ref.Vessel,ref.Pod,convert(nvarchar(10),ref.Eta,103) Eta,Convert(nvarchar(10),ref.Etd,103) Etd,job.JobNo,job.HblNo as HBL,(select Name from dbo.XXParty where PartyId=job.CustomerId) as CustomerName,job.Qty,job.Weight,job.volume,ref.StatusCode StatusCode,job.StatusCode as JobStatusCode from SeaExportRef ref left join SeaExport job on ref.RefNo=job.RefNo";
        ...
        {
            string sql_Con = string.Format("select distinct RefNo from SeaExportMkg where rTRIM(ContainerNo)='{0}'", txt_Cont.Text.Trim());
        ...
        else
        {
            ...
            if (dateFrom.Length > 0 && dateTo.Length > 0)
                where = GetWhere(where, string.Format(" ref.Etd >= '{0}' and ref.Etd <= '{1}'", dateFrom, dateTo));
            if (pod.Length > 0)
                where = GetWhere(where, " ref.Pod='" + pod + "'");
        }
    ...
    protected void grid_ref_HtmlRowPrepared(object sender, ASPxGridViewTableRowEventArgs e)
    {
        if (e.RowType == DevExpress.Web.ASPxGridView.GridViewRowType.Data)
        {
            ...
            }else 
            {
                ...
                DateTime etd = SafeValue.SafeDate(this.grid_ref.GetRowValues(0, "Etd"), new DateTime(1900, 1, 1));
                if ((DateTime.Today.Subtract(etd)).Days > 30)
