...
public partial class PagesFreight_ExpFclQuote : BasePage
{
...
FROM SeaQuote1 AS q INNER JOIN XXParty AS c ON q.PartyTo = c.PartyId
WHERE     (c.SalesmanId = '" + userName + "') and " + where).Tables[0];
                ...
                where1 += ")";
                this.dsQuotation.FilterExpression = where1;
            }
            else
