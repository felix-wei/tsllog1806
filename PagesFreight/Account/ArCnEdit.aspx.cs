...
public partial class Account_ArCnEdit : BasePage
{
    protected void page_Init(object sender, EventArgs e)
    {
        ...
        if (Session["SeaCnEditWhere"] != null)
        {
            this.dsArInvoice.FilterExpression = Session["SeaCnEditWhere"].ToString();
...
WHERE (det.DocId = '{0}') and (det.DocType='CN')", docId)), 0);
    ...
    private void UpdateArInv(string docNo, string docType)
    {
        string sql_invoice = "update XAArInvoice set ExportInd='Y' where DocNo='" + docNo + "' AND DocType='" + docType + "'";
