<%@ Page Language="C#" AutoEventWireup="true" CodeFile="UpdateTransport2_Planner.aspx.cs" Inherits="PagesContTrucking_Daily_UpdateTransport2_Planner" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml" ng-app="app">
<head runat="server">
    <title>OP TPT Planner</title>
    ...
    <script type="text/javascript">
        app.factory('SV_Job', function ($http, SV_Common) {
            ...
            var vm = {
                init: function () {
                    SV_Common.setWebService('WebService_UpDateTransport2.asmx');
                    data.search = {
                        From: new Date(moment().add(0, 'days')),
                ...
                refresh: function (par, cb) {
                    ...
                    var func = "/PlannerTPTList_GetData";
                    SV_Common.http(func, pars, function (res) {
                        if (res.status == '1') {
                            //console.log(res);
                            var temp = res.context;

                            data.leftList = temp.dataList;
                            data.rightList = temp.rightList;
                            //data.leftList = [];
                            //data.rightList = [];

                            //angular.forEach(temp.dataList, function (row) {
                            //    if (row.DriverCode3 == '') {
                            //        data.leftList.push(row);
                            //    } else {
                            //        data.rightList.push(row);
                            //    }
                            //});
                            var rightList = data.rightList;
                            var newList = [];
                            angular.forEach(temp.driverList, function (row) {
                                var newD = { isTitle: true, text: row.Code, Code: row.Code, sum: 0 };
                                newList.push(newD);
                                angular.forEach(rightList, function (rl_row) {
                                    if (rl_row.DriverCode3 == row.Code) {
                                        newList.push(rl_row);
                                        newD.sum++;
                                    }
                                });
            ...
            .controller('Ctr_Job', function ($scope, SV_Job, $timeout, SV_Popup) {
                ...
                $scope.floatView = {
                    ...
                    selectDriver11: function () {
                        SV_Job.refresh_masterdata_driver();
                        $scope.masterData.show($scope.vm.masterData.driver, $scope.floatView.selectDriver11_callback);
                    },
                    selectDriver11_callback: function (res) {
                        //console.log(res);
                        $scope.vm.curView.mast.DriverCode11 = res.c;
                        $scope.floatView.changeView();
                    },
                    selectDriver12: function () {
                        SV_Job.refresh_masterdata_driver();
                        $scope.masterData.show($scope.vm.masterData.driver, $scope.floatView.selectDriver12_callback);
                    },
                    selectDriver12_callback: function (res) {
                        //console.log(res);
                        $scope.vm.curView.mast.DriverCode12 = res.c;
                        $scope.floatView.changeView();
                    },
...
<body ng-controller="Ctr_Job">
    ...
    <div class="joblist_content joblist_content_top35 {{partDetail.data.css}}">
            ...
            <div class="part1" ng-class="{'M1':'part1M1','M2':'part1M2','M3':'part1M3','M4':'part1M4'}[vm.search.viewMode]">
                <table class="bx_table_grid">
                    <tr>
                        ...
                        <th>Permit/SEF.NO</th>
                        <th>ContainerNo/SealNo</th>
                    </tr>
                    <tr ng-repeat="row in vm.leftList">
                        ...
                        <td>{{row.PermitNo}}/{{row.ClientRefNo}}</td>
                        <td>{{row.ContainerNo}}
                            <br />
                            /{{row.SealNo}}<br />
            ...
            <div class="part2" ng-class="{'M1':'part2M1','M2':'part2M2','M3':'part2M3','M4':'part2M4'}[vm.search.viewMode]">
                <table class="bx_table_grid">
                    ...
                    <tr ng-repeat="row in vm.rightList">
                        <td ng-show="row.isTitle" style="background-color: #aaaaaa; padding: 6px;" colspan="11">
                        ...
                        <td ng-hide="row.isTitle">Job:{{row.JobRemark}}</td>
                        <td ng-hide="row.isTitle">
                            <div class="col_boxset col_boxset_p">
                                <div class="col_box_small col_box_small_p {{row.JobType=='IMP'?'col_box_small_p_grey':(row.JobType=='EXP'?'col_box_small_p_red':'')}}" ng-class="{'P':'col_box_orange','S':'col_box_green','C':'col_box_blue','X':'col_box_gray','A':'col_box_calm'}[row.Statuscode]" ng-click="action.viewRow(row);">
                                    &nbsp;{{row.TripCode}}
                        ...
                        <td ng-hide="row.isTitle">{{row.PermitNo}}/{{row.ClientRefNo}}</td>
                        <td ng-hide="row.isTitle">{{row.ContainerNo}}<br />
                            /{{row.SealNo}}<br />
        ...
        <div class="joblist_content_detail">
            ...
            <div class="body" ng-include="'UpdateTransport2_Detail.aspx'">
            </div>
