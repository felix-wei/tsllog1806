...
public partial class PagesContTrucking_SelectPage_SelectStock : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                type = "WGR";
                lbl_ContNo.Visible = false;
                cbb_ContNo.Visible = false;
            }
            if (no.Length > 0) {
                dsCont.FilterExpression = "JobNo='"+no+"'";
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (id != null && isPay != null && isPay.Checked)
            {
                list.Add(new Record(SafeValue.SafeInt(id.Text, 0), SafeValue.SafeDecimal(spin_Qty.Value), SafeValue.SafeDecimal(spin_WholeQty.Value), 
                    ...
                    SafeValue.SafeDecimal(spin_WidthPack.Value), SafeValue.SafeDecimal(spin_HeightPack.Value),lbl_JobNo.Text));
            }
            else if (id == null)
                break;
        }
                ...
                if (list.Count > 0)
                {
                    ...
                    for (int i = 0; i < list.Count;i++ )
                    {
                        ...
                        if (qty > 0)
                        {
                            ...
                            house.LineId = id;
                            house.ContId = SafeValue.SafeInt(cbb_ContNo.Value,0);
    ...
    public decimal BalanceQty(object inventoryId)
    {
        string sql = string.Format(@"select tab_bal.BalQty  from job_house mast
left join (select  sum(case when CargoType='IN' then QtyOrig else -QtyOrig end) as BalQty,InventoryId from job_house  group by InventoryId) as tab_bal on tab_bal.InventoryId=mast.InventoryId
where mast.InventoryId='{0}'", inventoryId);
        return SafeValue.SafeDecimal(ConnectSql.ExecuteScalar(sql));
    }
    public decimal BalanceSkuQty(object inventoryId)
    {
        string sql = string.Format(@"select tab_bal.BalQty  from job_house mast
left join (select  sum(case when CargoType='IN' then PackQty else -PackQty end) as BalQty,InventoryId from job_house  group by InventoryId) as tab_bal on tab_bal.InventoryId=mast.InventoryId
where mast.InventoryId='{0}'", inventoryId);
        return SafeValue.SafeDecimal(ConnectSql.ExecuteScalar(sql));
    }
    public decimal BalanceWeight(object inventoryId)
    {
        string sql = string.Format(@"select tab_bal.BalQty  from job_house mast
left join (select  sum(case when CargoType='IN' then WeightOrig else -WeightOrig end) as BalQty,InventoryId from job_house group by InventoryId) as tab_bal on tab_bal.InventoryId=mast.InventoryId
where mast.InventoryId='{0}'", inventoryId);
        return SafeValue.SafeDecimal(ConnectSql.ExecuteScalar(sql));
    }
    public decimal BalanceVolume(object inventoryId)
    {
        string sql = string.Format(@"select tab_bal.BalQty  from job_house mast
left join (select  sum(case when CargoType='IN' then VolumeOrig else -VolumeOrig end) as BalQty,InventoryId from job_house  group by InventoryId) as tab_bal on tab_bal.InventoryId=mast.InventoryId
where mast.InventoryId='{0}'", inventoryId);
        return SafeValue.SafeDecimal(ConnectSql.ExecuteScalar(sql));
    }
...
left join (select  sum(case when CargoType='IN' then QtyOrig else -QtyOrig end) as BalQty,InventoryId from job_house where  CargoStatus='C' group by InventoryId) as tab_bal on tab_bal.InventoryId=mast.InventoryId
where job.PartyId='{0}' and CargoType='IN' and tab_bal.BalQty>0", client);
