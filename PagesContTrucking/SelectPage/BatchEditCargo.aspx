    ...
    <script type="text/javascript">
            ...
            var vm = {
                    ...
                    SV_Common.http(func, pars, function (res) {
                        if (res.status == true) {
                            ...
                            angular.forEach(temp, function (row) {
                                ...
                                row.Mft_LotDate = vm.FormatDate1(row.Mft_LotDate);
                                row.Mft_ExpiryDate = vm.FormatDate1(row.Mft_ExpiryDate);
                            });
                            ...
                            data.locked = res.context.locked;
                ...
                save: function (par, cb) {
                    ...
                    angular.forEach(pars.list, function (item) {
                        ...
                        item.Mft_LotDate = moment(item.Mft_LotDate).format("YYYY-MM-DD");
                        item.Mft_ExpiryDate = moment(item.Mft_ExpiryDate).format("YYYY-MM-DD");
                    })
                    pars.Client = par3.value;
                    var func = "/Save_Data";
                    SV_Common.http(func, pars, function (res) {
                ...
                locked: function (par, cb) {
                    var pars = angular.copy(data.search);
                    pars.list = angular.copy(par)
                    var func = "/Locked_Data";
                    SV_Common.http(func, pars, function (res) {
                        if (cb) {
                            vm.init();
                            cb(res);
                ...
                unlocked: function (par, cb) {
                    var pars = angular.copy(data.search);
                    pars.list = angular.copy(par)
                    var func = "/UnLocked_Data";
                    SV_Common.http(func, pars, function (res) {
                        if (cb) {
                            vm.init();
                            cb(res);
                ...
                copy_from_job: function (par,cb) {
                    var pars = angular.copy(data.search);
                    var par1 = document.getElementById('JobNo');
                    pars.JobNo = data.JobNo;
                    pars.NewJob = par1.value;
                    var func = "/Copy_Data_From_JobNo";
                    SV_Common.http(func, pars, function (res) {
                        if (cb) {
                            vm.init();
                            cb(res);
        ...
        .controller('Ctrl_CargoController', function ($scope, SV_CargoService, $timeout, SV_Popup) {
            ...
            $scope.init = function () {
                ...
                $scope.cnt = parseFloat($scope.vm.locked);
                var par1 = document.getElementById('JobNo');
                var par2 = document.getElementById('CargoType');
            ...
            $scope.action = {
                ...
                locked_data: function () {
                    SV_CargoService.locked($scope.vm.list, $scope.action.locked_data_callback);
                },
                locked_data_callback: function (res) {
                    if (res.status == true) {
                        if (parent.parent.notice) {
                            parent.parent.notice('Locked Successful！');
                        }
                    }
                },
                unlocked_data: function () {
                    SV_CargoService.unlocked($scope.vm.list, $scope.action.unlocked_data_callback);
                },
                unlocked_data_callback: function (res) {
                    if (res.status == true) {
                        if (parent.parent.notice) {
                            parent.parent.notice('UnLocked Successful！');
                        }
                    }
                },
                ...
                copy_from_job: function () {
                    SV_CargoService.copy_from_job($scope.vm.list, $scope.action.copy_from_job_callback);
                },
                copy_from_job_callback: function (res) {
                    if (res.status == true) {
                        if (parent.parent.notice) {
                            parent.parent.notice('Copy Successful！');
                        }
                    }
                },
                upload: function () {
                    var obj = document.getElementById('JobNo');
                    popubCtr.SetHeaderText('Import Data ');
                    popubCtr.SetContentUrl('/PagesContTrucking/ImportData.aspx?no=' + obj.value+'&action=WGR'+'&type=IN');
                    popubCtr.Show();
                    //$('#btn').addClass('hide');
                }
            }
    ...
    <form id="form1" runat="server">
        <div class="form">
            <div class="body">
                ...
                <div class="table table-hover table-responsive btn_div">
                    <div id="btn">
                        <div id="add" style="display: none" class="btn_left">
                            <input type="button" class="single_button_110 button" value="Add New" ng-click="action.add_data();" />
                        </div>

                        <%-- <div class="btn_left1" id="lock" style="display:none">
                        <div ng-show="vm.locked==0">
                            <input type="button" class="single_button_110 button" value="Locked All" ng-click="action.locked_data();" />
                        </div>
                        <div ng-show="vm.locked>0">
                             <input type="button" class="single_button_110 button" value="UnLocked All" ng-click="action.unlocked_data();" />
                        </div>
                    </div>--%>

                        <div class="btn_left1" id="keyin">
                            <input type="button" class="single_button_110 button" value="Keyin for Container" ng-click="action.keyin_for_cont();" />
                        </div>
                        <div id="upload" style="display: none" class="btn_left3">
                            <input type="button" class="single_button_110 button" value="Batch Import" ng-click="action.upload();" />
                        </div>
                        <%-- <div  class="btn_left3" id="copy"  style="display:none">
                        <table>
                            <tr>
                                <td>
                                    JobNo
                                ...
                                <td>
                                    <input  type="text" ng-model="vm.JobNo" class="single_text_full"/>
                                </td>
                                <td><input type="button" class="single_button_110 button" value="Copy Cargo" ng-click="action.copy_from_job();" /></td>
                            </tr>
                        </table>
                    </div>--%>
                        <div class="btn_right1" id="save">
                            <input type="button" class="single_button_110 button" value="Save" ng-click="action.save_data();" />
                        </div>
                        <div class="btn_right" id="return">
                            <input type="button" class="single_button_110 button" value="Return Job" ng-click="action.return_job();" />
                        </div>
                    </div>
                </div>
                <table class="table table-hover table-responsive table-bordered hidden">
                    <tr>
                        <td>
                ...
                <table class="table table-hover table-responsive table-bordered">
                    <thead>
                        <tr style="background: #ccc">
                            ...
                            <th width="180" colspan="6">Pallet/Carton/Mft</th>
                    ...
                    <tbody>
                        <tr ng-show="vm.list.length>0" ng-repeat="h in vm.list">
                            ...
                            <td>
                                <input type="text" class="width-140 control" ng-model="h.PalletNo" />

                            </td>

                            <td>
                                <input type="text" class="width-140 control" ng-model="h.CartonNo" />

                            </td>

                            <td>
                                <input type="text" class="width-140 control" ng-model="h.Mft_LotNo" />

                            </td>

                            <td>
                                <input type="date" ng-model="h.Mft_LotDate" class="width-140 control" />
                            </td>

                            <td>
                                <input type="date" ng-model="h.Mft_ExpiryDate" class="width-140 control" />
                            </td>

                            <td>
                                <select ng-model="h.OnHold" class="width-110 control">
