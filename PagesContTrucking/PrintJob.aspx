...
<%
    ...
    string sql_sumx =  @"select Category, sum(LocAmt1) as LocAmt1, Sum(GstAmt) as GstAmt, Sum(Gst2Amt) as Gst2Amt, Sum(LocAmt2) as LocAmt2 from (
            select '1.Income' as Category, sum( Case d.DocType When 'CN' Then -1*(l.LocAmt-l.GstAmt) ELSE (l.LocAmt-l.GstAmt) END) as LocAmt1, 0 as LocAmt, 0 as LocAmt2, 0 as GstAmt,0 AS Gst2Amt from xaarinvoice d, xaarinvoicedet l where d.sequenceid=l.docid and d.mastrefno='" + job_no + "' " 
            + " union all " + 
            " select '2.Expense' as Category, sum( Case d.DocType When 'SC' Then 1*l.LocAmt ELSE -1*l.LocAmt END) as LocAmt1, 0 as LocAmt, 0 as LocAmt2, 0 as GstAmt, 0 AS Gst2Amt from xaappayable d, xaappayabledet l where d.partyto<>'PSAC' and d.SequenceId=l.DocId and l.MastRefNo='" + job_no + "' " 
            + " union all " + 
            ...
            + " union all " +
            "select '2.Expense' as Category, -1*(select  sum(LocAmt) as price from job_cost where  chgcode in ('TC31x','TC41x','TC42x') and LineType='CONT' and  JobNo='" + job_no + "' group by JobNo) as LocAmt1, 0 as LocAmt,  0 as LocAmt2, 0 as GstAmt, 0 as Gst2Amt from ctm_job l where l.jobno='" + job_no + "' "
            + " union all " +
            "select '2.Expense' as Category,0 as LocAmt1, 0 as LocAmt,  0 as LocAmt2, 0 as GstAmt, -1*(select  sum(price) as price from job_cost where  LineType='CL' and tripno=l.id and JobNo='" + job_no + "' group by JobNo) as Gst2Amt from ctm_jobdet2 l where l.jobno='" + job_no + "' "
            + " union all " +
            "select '2.Expense' as Category, 0 as LocAmt1,0 as LocAmt, -1*(select  sum(price) as price from job_cost where  LineType='DP' and tripno=l.id and JobNo='" + job_no + "' group by JobNo) as LocAmt2, 0 as GstAmt, 0 as Gst2Amt from ctm_jobdet2 l where l.jobno='" + job_no + "' ) as Tmp group by Category  "
		;
    string sql_sum  =  @"select Category, sum(LocAmt1) as LocAmt1, Sum(GstAmt) as GstAmt, Sum(Gst2Amt) as Gst2Amt, Sum(LocAmt2) as LocAmt2 from (
            ...
            " select '2.Expense' as Category, sum( Case d.DocType When 'SC' Then 1*l.LocAmt ELSE -1*l.LocAmt END) as LocAmt1, 0 as LocAmt, 0 as LocAmt2, 0 as GstAmt, 0 AS Gst2Amt from xaappayable d, xaappayabledet l where d.partyto<>'PSAC' and d.SequenceId=l.DocId and l.MastRefNo='" + job_no + "' " 
            + " union all " + 
            " select '2.Expense' as Category, 0 as LocAmt1, 0 as LocAmt, 0 as LocAmt2, sum( Case d.DocType When 'SC' Then 1*l.LocAmt ELSE -1*l.LocAmt END) as GstAmt, 0 AS Gst2Amt from xaappayable d, xaappayabledet l where d.partyto='PSAC' and l.locamt>0 and d.SequenceId=l.DocId and l.MastRefNo='" + job_no + "' " 
            + " union all " + 
            " select '1.Income' as Category, 0 as LocAmt1, 0 as LocAmt, 0 as LocAmt2, sum( Case d.DocType When 'SC' Then 1*l.LocAmt ELSE -1*l.LocAmt END) as GstAmt, 0 AS Gst2Amt from xaappayable d, xaappayabledet l where d.partyto='PSAC' and l.locamt<0 and d.SequenceId=l.DocId and l.MastRefNo='" + job_no + "' " 
           + " union all " +
            "select '2.Expense' as Category, -1*(select  sum(LocAmt) as price from job_cost where  chgcode in ('TC31x','TC41x','TC42x') and LineType='CONT' and  JobNo='" + job_no + "' group by JobNo) as LocAmt1, 0 as LocAmt,  0 as LocAmt2, 0 as GstAmt, 0 as Gst2Amt from ctm_job l where l.jobno='" + job_no + "' "
            + " union all " +
            "select '2.Expense' as Category,0 as LocAmt1, 0 as LocAmt,  0 as LocAmt2, 0 as GstAmt, -1*(select  sum(price) as price from job_cost where  LineType='CL' and tripno=l.id and JobNo='" + job_no + "' group by JobNo) as Gst2Amt from ctm_jobdet2 l where l.jobno='" + job_no + "' "
...
<%
    DataTable dt_co0 = ConnectSql.GetTab("select d.DocNo,d.DocType,d.DocDate,(CASE d.DocType When 'SC' Then -1 Else 1 End) * isnull((select sum(locamt-gstamt) from xaarinvoicedet where docno=d.docno),0) as LocAmt,d.Description,p.Name as PartyName, d.EntryDate from xaarinvoice d, xxparty p where d.partyto=p.partyid and d.mastrefno='" + job_no + "' order by docno");
...
<tr>
    ...
    <%
        if(S.Text(dt_co0.Rows[i]["DocType"]) == "CN")
...
<%
    dt_co0 = ConnectSql.GetTab("select * from job_cost where jobno='" + job_no + "' and linetype='CONT' and chgcode in ('TC31x','TC41x','TC42x') ");
...
<tr>
<td colspan=14 >
<span style="font-size:14px;font-weight:bold">Job Costing</span>
 
</td>
</tr>
<tr class=head>
    <td class="w120">Charge Code</td>
    <td class="w200">Description</td>
    <td class="w120">Cont/HBL No</td>
    <td class="w120">Total</td>
    <td class="w120">Updated</td>
...
for (int i = 0; i < dt_co0.Rows.Count; i++)
{
    ...
    %>
<tr>
    <td><%= dt_co0.Rows[i]["ChgCode"]%></td>
    <td><%= dt_co0.Rows[i]["ChgCodeDes"]%></td>
    <td><%= dt_co0.Rows[i]["ContNo"]%> / <%= dt_co0.Rows[i]["ContType"]%></td>
    <td class=right><%= string.Format("{0:0.00}", SafeValue.SafeDecimal(dt_co0.Rows[i]["LocAmt"]))%></td>
    <td class=right></td>
</tr>
    <%
