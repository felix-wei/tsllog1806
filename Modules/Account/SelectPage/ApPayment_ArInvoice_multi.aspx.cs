...
public partial class PagesAccount_SelectPage_ApPayment_ArInvoice_multi : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["partyTo"] != null && Request.QueryString["no"] != null)
            {
                ...
                string sql = "select CustId from XXCustVendor where VendorId='" + partyTo + "'";
                ...
                repNo = Request.QueryString["no"].ToString();
                 ...
                 sql = string.Format(@"SELECT TOP (100) SequenceId, DocType, DocNo, DocDate,MastType, CurrencyId, ExRate, DocAmt, LocAmt, BalanceAmt
FROM XAArInvoice WHERE (ExportInd = 'Y') AND (BalanceAmt > 0) AND {0}", partyTo);
