...
public partial class PagesAccount_SelectPage_ApPayment_ApPayable_multi : BasePage
{
	...
	public decimal exrate0 = 0;
	public string curr0 = "SGD";
    protected void Page_Init(object sender, EventArgs e)
    {
               string repNo = Request.QueryString["no"] ?? "0";
			   if(repNo.Length > 3) {
					exrate0 = S.Decimal(D.One("select top 1 exrate from xaappayment where sequenceid='"+repNo+"'"));
					curr0 = S.Text(D.One("select top 1 currencyid from xaappayment where sequenceid='"+repNo+"'"));
				}
				//throw new Exception(exrate0.ToString());
	}
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["partyTo"] != null && Request.QueryString["no"] != null)
            {
				...
				//exrate0 = S.Decimal(D.One("select top 1 exrate from xaappayment where docno='"+repNo+"'"));
                string excetpPl = SafeValue.SafeString(System.Configuration.ConfigurationManager.AppSettings["ExceptPl"], "");
                //this.dsApPayable.FilterExpression = "BalanceAmt!=0 and ExportInd='Y' and (DocType='PIN' OR DocType='PDN' OR DocType='PCN') and PartyTo='" + partyTo + "'";
                //if (excetpPl.Length > 0)
                //    this.dsApPayable.FilterExpression += " and DocNo Not In('" + excetpPl.Replace(",", "','") + "0')";

                string sql = string.Format(@"SELECT  SequenceId, DocType, DocNo, DocDate,MastType,SupplierBillNo,SupplierBillDate, CurrencyId, ExRate, DocAmt, LocAmt, BalanceAmt, BalanceAmt as BalanceAmt1,CASE DocType When 'PCN' Then -1 * BalanceAmt Else BalanceAmt End as BalanceAmt1, CASE DocType When 'PCN' Then -1 * DocAmt Else DocAmt End as DocAmt1
FROM XAApPayable WHERE (ExportInd = 'Y') AND (BalanceAmt <> 0) and (DocType='PIN' OR DocType='PDN' OR DocType='PCN') and PartyTo='{0}'", partyTo);
                if (excetpPl.Length > 0)
    ...
    protected void btn_Sch_Click(object sender, EventArgs e)
    {
        ...
        string sql = string.Format(@"SELECT TOP (100) SequenceId, DocType, DocNo, DocDate,MastType,SupplierBillNo,SupplierBillDate, CurrencyId, ExRate, DocAmt, LocAmt, BalanceAmt, BalanceAmt as BalanceAmt1,CASE DocType When 'PCN' Then -1 * BalanceAmt Else BalanceAmt End as BalanceAmt1, CASE DocType When 'PCN' Then -1 * DocAmt Else DocAmt End as DocAmt1
FROM XAApPayable WHERE (ExportInd = 'Y') AND (BalanceAmt <> 0) and (DocType='PIN' OR DocType='PDN' OR DocType='PCN') and PartyTo='{0}' ", partyTo);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                try
                {
                    ...
                    if (tab.Rows.Count == 1)
                    {
                        ...
                        repDet.PayNo = "";
                        repDet.PayType = "";
                        repDet.Remark1 = "" + supplierBillNo;
                        repDet.Remark2 = "AcCode -" + acCode;
                        repDet.Remark3 = " ";
...
WHERE (det.DocId = '{0}')and (det.DocType='PIN' or det.DocType='PCN' or det.DocType='PDN')", siId)), 0);
...
WHERE (det.DocId = '{0}')and (det.DocType='PIN' or det.DocType='PCN' or det.DocType='PDN')", siId)), 0);
    ...
    internal class Record
    {
        ...
        public Record(int _docId, decimal _payAmt , decimal _payAmt2)
        {
            docId = _docId;
            payAmt = _payAmt;
            payAmt2 = _payAmt2;
            ...
            if (docId != null && payAmt != null && isPay != null && isPay.Checked)
            {
                if (SafeValue.SafeDecimal(payAmt.Value, 0) != 0 )
                {
                    ...
                    list.Add(new Record(SafeValue.SafeInt(docId.Text, 0),
                        ...
                        SafeValue.SafeDecimal(payAmt2.Value, 0)
                        ));
                }
            }
        }
