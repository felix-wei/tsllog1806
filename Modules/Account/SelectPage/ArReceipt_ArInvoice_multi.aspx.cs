...
public partial class PagesAccount_SelectPage_ArReceipt_ArInvoice_multi : BasePage
{

        public string partyTo = "";
        public string repNo = "";

	public decimal GetUseAmt(decimal amt,decimal ex1)
	{
		decimal ret = 0;
		//decimal ex2 = 1; //S.Decimal(D.One("select DocExRate from XaArReceipt Where DocNo='"+repNo+"'"));
		ret = SafeValue.ChinaRound(amt * ex1,2); 
		return ret;
	}
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["partyTo"] != null && Request.QueryString["no"] != null)
            {
                ...
                string sql = string.Format(@"SELECT SequenceId, DocType, DocNo, DocDate,MastType, CurrencyId, ExRate, DocAmt, LocAmt, BalanceAmt,BalanceAmt as BalanceAmt1,CASE DocType When 'SCN' Then -1 * BalanceAmt Else BalanceAmt End as BalanceAmt1, CASE DocType When 'SCN' Then -1 * DocAmt Else DocAmt End as DocAmt1
FROM XAArInvoice WHERE (ExportInd = 'Y') AND (BalanceAmt <> 0) AND PartyTo='{0}'   ", partyTo);

				sql += " order by doctype, docdate,docno";
				//throw new Exception(sql);
                DataTable tab1 = C2.Manager.ORManager.GetDataSet(sql).Tables[0];
				string qcurr = Request.QueryString["curr"] ?? "SGD";
				for(int rr=0;rr<tab1.Rows.Count;rr++)
				{
					if(S.Text(tab1.Rows[rr]["CurrencyId"]) != qcurr)
						tab1.Rows[rr]["BalanceAmt1"] = 0;
				}
	...
	protected void bal_amt_init(object sender, EventArgs e)
    {
        ASPxSpinEdit editor = sender as ASPxSpinEdit;
        GridViewDataRowTemplateContainer container = editor.NamingContainer as GridViewDataRowTemplateContainer;

        editor.ClientInstanceName = String.Format("editor{0}", container.VisibleIndex);
        editor.ClientSideEvents.ValueChanged = String.Format("function (s, e) {{ copier{0}.SetNumber(editor{0}.GetNumber()); }}", container.VisibleIndex);
    }
	
	protected void bal_copy_init(object sender, EventArgs e)
    {
        ASPxSpinEdit editor = sender as ASPxSpinEdit;
        GridViewDataRowTemplateContainer container = editor.NamingContainer as GridViewDataRowTemplateContainer;

        editor.ClientInstanceName = String.Format("copier{0}", container.VisibleIndex);
        //editor.ClientSideEvents.TextChanged = String.Format("function (s, e) {{ copier{0}.SetText(editor{0}.GetText(); }}", container.VisibleIndex);
    }
    ...
    protected void btn_Sch_Click(object sender, EventArgs e)
    {
        ...
        string sql = string.Format(@"SELECT TOP (120) SequenceId, DocType, DocNo, DocDate,MastType, CurrencyId, ExRate, DocAmt, LocAmt, BalanceAmt, BalanceAmt as BalanceAmt1, CASE DocType When 'SCN' Then -1 * BalanceAmt Else BalanceAmt End as BalanceAmt1, CASE DocType When 'SCN' Then -1 * DocAmt Else DocAmt End as DocAmt1
FROM XAArInvoice WHERE (ExportInd = 'Y') AND (BalanceAmt <> 0) AND PartyTo='{0}'", partyTo);
        ...
        {
            sql += string.Format(" and DocDate >= '{0:yyyy-MM-dd}' and DocDate<'{1:yyyy-MM-dd}'", this.txt_FromDt.Date, this.txt_toDt.Date.AddDays(1));
        }
		//throw new Exception(sql);
		sql += " Order By DocType,DocDate ";
        DataTable tab1 = C2.Manager.ORManager.GetDataSet(sql).Tables[0];
            ...
            if (true)
            {
                try
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        decimal payAmt2 = list[i].payAmt2;
                        ...
                        if (tab.Rows.Count == 1)
                        {
                            ...
                            //repDet.LineDocAmt = payAmt2;// payAmt;
                            ...
                            repDet.Remark1 = docNo;
                            repDet.Remark2 = acCode;
                            repDet.Remark3 = " ";
...
WHERE     (det.DocId = '{0}') and (det.DocType='SCN')", siId)), 0);
...
WHERE (det.DocId = '{0}') and (det.DocType='SCN')", siId)), 0);
    ...
    private decimal PayLocAmt(string siId)
    {
        decimal payLocAmt = SafeValue.SafeDecimal(Manager.ORManager.ExecuteScalar(string.Format(@"SELECT sum(det.LocAmt)
FROM  XAArReceiptDet AS det INNER JOIN XAArReceipt AS mast ON det.RepId = mast.SequenceId
WHERE (det.DocId = '{0}') and (det.DocType='SIN' or det.DocType='SDN')", siId)), 0);
        payLocAmt += SafeValue.SafeDecimal(Manager.ORManager.ExecuteScalar(string.Format(@"SELECT  sum(det.LocAmt)
FROM XAApPaymentDet AS det INNER JOIN  XAApPayment AS mast ON det.PayId = mast.SequenceId
WHERE (det.DocId = '{0}') and (det.DocType='SIN' or det.DocType='SDN')", siId)), 0);
        return payLocAmt;
    ...
    internal class Record
    {
        ...
        public decimal payAmt2 = 0;
        public bool isPay = false;
        public Record(string _docId, decimal _payAmt, decimal _payAmt2)
        {
            docId = _docId;
            payAmt = _payAmt;
            payAmt2 = _payAmt2;
    ...
    private void OnLoad(string r)
    {
        ...
        for (int i = start; i < end; i++)
        {
            ...
            ASPxSpinEdit payAmt2 = this.ASPxGridView1.FindRowTemplateControl(i, "spin_Amt2") as ASPxSpinEdit;
            ...
            if (docId != null && payAmt != null && isPay != null && isPay.Checked)
            {
                if (SafeValue.SafeDecimal(payAmt.Value, 0) != 0 ) //&& SafeValue.SafeDecimal(payAmt.Value, 0) <= SafeValue.SafeDecimal(balanceAmt.Value, 0))
                {
                    ...
                    list.Add(new Record(docId.Text, S.Decimal(payAmt.Value),S.Decimal(payAmt2.Value)));
                }
            }
        }
    }
