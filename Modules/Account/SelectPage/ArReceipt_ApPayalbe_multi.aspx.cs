...
public partial class PagesAccount_SelectPage_ArReceipt_ApPayalbe_multi : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["partyTo"] != null && Request.QueryString["no"] != null)
            {
                ...
                string sql = "select VendorId from XXCustVendor where CustId='" + partyTo + "'";
                DataTable tab = C2.Manager.ORManager.GetDataSet(sql).Tables[0];
                if (tab.Rows.Count > 0)
                {
                    partyTo = string.Format("( PartyTo='{0}'", partyTo);
                    for (int i = 0; i < tab.Rows.Count; i++)
                    {
                        partyTo += string.Format(" or PartyTo ='{0}'", tab.Rows[i][0]);
                    }
                    partyTo += ")";
                }
                else
                    partyTo = string.Format(" PartyTo='{0}'", partyTo);
                sql = string.Format(@"SELECT TOP (100) SequenceId, DocType, DocNo, DocDate,MastType,SupplierBillNo,SupplierBillDate, CurrencyId, ExRate, DocAmt, LocAmt, BalanceAmt
FROM XAApPayable WHERE (ExportInd = 'Y') AND (BalanceAmt > 0) and (DocType='PIN' OR DocType='PDN' OR DocType='PCN') and {0}", partyTo);
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    try
                    {
                        ...
                        if (tab.Rows.Count == 1)
                        {
                            ...
                            repDet.RepLineNo = SafeValue.SafeInt(C2.Manager.ORManager.ExecuteScalar("select count(*) from XAArReceiptDet where RepId='" + payId + "'"), 0) + 1;
                            repDet.RepId = SafeValue.SafeInt(payId,0);
