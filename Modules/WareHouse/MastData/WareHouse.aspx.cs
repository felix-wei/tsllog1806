...
public partial class WareHouse_MastData_WareHouse : System.Web.UI.Page
{
    ...
    protected void AddOrUpdate()
    {
        ...
        string sql =string.Format(@"select Code from ref_warehouse where Id!={0}",id);
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    throw new Exception("The Warehouse code is duplicate!");
                    return;
                }
        ...
        wh.IsBonded = cmb_Type.Text;
        ASPxTextBox txt_LotCode = pageControl.FindControl("txt_LotCode") as ASPxTextBox;
        wh.LotCode = txt_LotCode.Text;
        ASPxTextBox txt_LicenseNo = pageControl.FindControl("txt_LicenseNo") as ASPxTextBox;
        wh.LicenseNo = txt_LicenseNo.Text;
        ASPxDateEdit date_LicenseExpiry = pageControl.FindControl("date_LicenseExpiry") as ASPxDateEdit;
        wh.LicenseExpiry = date_LicenseExpiry.Date;
        ASPxMemo memo_LicenseRemark = pageControl.FindControl("memo_LicenseRemark") as ASPxMemo;
        wh.LicenseRemark = memo_LicenseRemark.Text;
