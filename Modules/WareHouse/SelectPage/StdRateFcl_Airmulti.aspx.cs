...
public partial class PagesFreight_SelectPage_AirStdRateFcl_multi : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                this.dsSeaQuoteDet.FilterExpression = " FclLclInd='Fcl' and  QuoteId='-1'";
            }
            else
        ...
        if (Request.QueryString["no"] != null)
        {
            ...
            try
            {
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    string sql = string.Format("SELECT ChgCode,ChgDes, Currency,ExRate, Qty,Price, Unit, MinAmt, Rmk,GstType,Gst,Amt FROM AirQuoteDet1 where SequenceId='{0}'", docId);
