...
public partial class Pages_DoOutEdit : System.Web.UI.Page
{
    ...
    protected void grid_Issue_InitNewRow(object sender, DevExpress.Web.Data.ASPxDataInitNewRowEventArgs e)
    {
        ...
        if (type == "ZG")
        {
            e.NewValues["WareHouseId"] = SafeValue.SafeString(System.Configuration.ConfigurationManager.AppSettings["DefaultBonded"]);
        }
        else
        {
            e.NewValues["WareHouseId"] = SafeValue.SafeString(System.Configuration.ConfigurationManager.AppSettings["DefaultWareHouse"]);
        }
        e.NewValues["TransportStatus"] = "New";
        e.NewValues["FreightStatus"] = "New";
    }
    protected string Save()
    {
        try
        {
            ...
            */
            ...
            if (memo_DeliveryTo.Text == "")
            {
                whDo.DeliveryTo = ConsigneeName.Text + "\n " + consigneeAdd.Text;
            }
            else {
            ...
            ASPxComboBox txt_UseTransport = pageControl.FindControl("txt_UseTransport") as ASPxComboBox;
            whDo.UseTransport = txt_UseTransport.Text;
            ASPxComboBox txt_TransportStatus = pageControl.FindControl("txt_TransportStatus") as ASPxComboBox;
            whDo.TransportStatus = txt_TransportStatus.Text;
            ASPxComboBox txt_UseFreight = pageControl.FindControl("txt_UseFreight") as ASPxComboBox;
            whDo.UseFreight = txt_UseFreight.Text;
            ASPxComboBox cmb_FreightStatus = pageControl.FindControl("cmb_FreightStatus") as ASPxComboBox;
            whDo.FreightStatus = cmb_FreightStatus.Text;
            ASPxDateEdit date_TransportStart = pageControl.FindControl("date_TransportStart") as ASPxDateEdit;
            whDo.TransportStart = date_TransportStart.Date;
            ASPxDateEdit date_TransportEnd = pageControl.FindControl("date_TransportEnd") as ASPxDateEdit;
            whDo.TransportEnd = date_TransportEnd.Date;
            ASPxTextBox txt_VechicleNo = pageControl.FindControl("txt_VechicleNo") as ASPxTextBox;
            whDo.VehicleNo = txt_VechicleNo.Text;
            ASPxMemo meno_Remark = pageControl.FindControl("meno_Remark") as ASPxMemo;
            whDo.Remarks = meno_Remark.Text;
            ASPxButtonEdit txt_TransportName = pageControl.FindControl("txt_TransportName") as ASPxButtonEdit;
            whDo.TransportName = txt_TransportName.Text;
            ASPxTextBox txt_TptJobNo = pageControl.FindControl("txt_TptJobNo") as ASPxTextBox;
            whDo.TptJobNo = txt_TptJobNo.Text;
            ...
            ASPxDateEdit date_Operation = grid_Issue.FindEditFormTemplateControl("date_Operation") as ASPxDateEdit;
            whDo.OperationStart = date_Operation.Date;
            ASPxTextBox txt_ContainerNo = this.grid_Issue.FindEditFormTemplateControl("txt_ContainerNo") as ASPxTextBox;
            whDo.ContainerNo = txt_ContainerNo.Text;
...
from Wh_DoDet2 where Id='{0}'", id, EzshipHelper.GetUserName());
        ...
        else if (s == "Costing")
        {
            #region Costing
            string result = "";
           string sql = string.Format(@"select sum(Qty1) as Qty from wh_dodet2 where DoNo='{0}' and DoType='OUT'", refN.Text);
            int sumQty = SafeValue.SafeInt(ConnectSql.ExecuteScalar(sql), 0);
            String[] chgCode = { "BGFEE", "HANDLE", "PERMIT", "STUFF20", "STUFF40", "ASN", "UNIQUE", "PALLET", "TPT", "FORKLIFT", "STORAGE", "CUSTOMS", "MISC", "ADHOC" };
            String[] chgCodeDes = { "BG USAGE", "WAREHOUSE HANDLING", "PERMIT", "STUFFING-1×20", "STUFFING-1×40", "ASN", "UNIQUE CREATE", "EMPTY PALLET", "TRANSPORT-PER TRIP", "FORKLIFT", "STORAGE", "CUSTOMS SUPERVISION", "MISC CHARGES", "ADHOC CHARGES" };
            for (int i = 0; i < chgCode.Length; i++)
            {
                string code = SafeValue.SafeString(chgCode[i]);
                string des = SafeValue.SafeString(chgCodeDes[i]);
                sql = string.Format(@"select count(*) from Wh_Costing where RefNo='{0}' and ChgCode='{1}' and JobType='OUT'", refN.Text, code);
               int cnt = SafeValue.SafeInt(ConnectSql.ExecuteScalar(sql), 0);
                if (cnt > 0)
                {
                    result += code + ",";
...
and wh_dodet2.doNo='{0}'", orderNo);
    ...
    protected void grid_SKULine_RowInserting(object sender, DevExpress.Web.Data.ASPxDataInsertingEventArgs e)
    {
        ...
        e.NewValues["Qty2"] = SafeValue.SafeDecimal(e.NewValues["Qty2"]);
        e.NewValues["Qty3"] = SafeValue.SafeInt(e.NewValues["Qty3"], 0);
        e.NewValues["Qty4"] = SafeValue.SafeDecimal(e.NewValues["Qty4"]);
        e.NewValues["Qty5"] = SafeValue.SafeDecimal(e.NewValues["Qty5"]);
    ...
    protected void grid_PickDetail_RowInserting(object sender, DevExpress.Web.Data.ASPxDataInsertingEventArgs e)
    {
        ...
        //    return;
        //}
        e.NewValues["Product"] = SafeValue.SafeString(e.NewValues["Product"]);
        ASPxButtonEdit refN = this.grid_Issue.FindEditFormTemplateControl("txt_DoNo") as ASPxButtonEdit;
        e.NewValues["DoNo"] = refN.Text;
        e.NewValues["DoType"] = "OUT";
        e.NewValues["Qty1"] = SafeValue.SafeDecimal(e.NewValues["Qty1"]);
    ...
    protected void grid_PickDetail_RowDeleting(object sender, DevExpress.Web.Data.ASPxDataDeletingEventArgs e)
    {
        ...
        string des = SafeValue.SafeString(e.Values["Des1"]);
        string remark = SafeValue.SafeString(e.Values["Remark"]);
        string lotNo = SafeValue.SafeString(e.Values["LotNo"]);
        string palletNo = SafeValue.SafeString(e.Values["PalletNo"]);
        ...
        {
           int result= UpdateMast(refN.Text,code,qty,des,remark,lotNo,palletNo);
           if (result > 0)
           {
    ...
    private int UpdateMast(string doNo,string sku,int qty,string des,string remark,string lotNo,string palletNo)
    {
        int result = 0;
        string sql =string.Format(@"select sum(Qty1) as PQty from wh_dodet2  where DoNo='{0}' and isnull(Product,'')='{1}' and isnull(LotNo,'')='{2}' and isnull(Des1,'')='{3}' and isnull(Remark,'')='{4}' and isnull(PalletNo,'')='{5}'",doNo,sku,lotNo,des,remark,palletNo);
        int totalQty = SafeValue.SafeInt(ConnectSql.ExecuteScalar(sql),0);
        if (totalQty > 0)
        {
            sql = string.Format(@"update wh_dodet set Qty1=Qty4-({0}+Qty5),JobStatus='Pending' where DoNo='{1}' and ProductCode='{2}' and isnull(LotNo,'')='{3}' and isnull(Des1,'')='{4}' and isnull(Remark,'')='{5}' and isnull(PalletNo,'')='{6}'", qty, doNo, sku, lotNo, des, remark, palletNo);
            result=SafeValue.SafeInt(ConnectSql.ExecuteSql(sql),0);

            sql = string.Format(@"update wh_dodet set Qty5=Qty4-Qty1,JobStatus='Pending' where DoNo='{1}' and ProductCode='{2}' and isnull(LotNo,'')='{3}' and isnull(Des1,'')='{4}' and isnull(Remark,'')='{5}' and isnull(PalletNo,'')='{6}'", qty, doNo, sku, lotNo, des, remark, palletNo);
            result = SafeValue.SafeInt(ConnectSql.ExecuteSql(sql), 0);
        }
        //else if (totalQty == 0)
        //{
    ...
    {
        ASPxButtonEdit txt_RefNo = this.grid_Issue.FindEditFormTemplateControl("txt_DoNo") as ASPxButtonEdit;
        e.NewValues["Qty"] = 1;
        ...
        e.NewValues["CreateDateTime"] = DateTime.Today;
        e.NewValues["PkgType"] = " ";
        e.NewValues["JobStart"] = DateTime.Now;
        e.NewValues["JobEnd"] = DateTime.Now;
    }
    protected void Grid_Cont_RowInserting(object sender, DevExpress.Web.Data.ASPxDataInsertingEventArgs e)
    {
        ...
        e.NewValues["CreateBy"] = userId;
        e.NewValues["CreateDateTime"] = DateTime.Today;
        e.NewValues["JobStart"] = SafeValue.SafeDate(e.NewValues["JobStart"], DateTime.Today);
        e.NewValues["JobEnd"] = SafeValue.SafeDate(e.NewValues["JobEnd"], DateTime.Today);
